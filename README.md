...
- **Default input cap is 96K tokens** (`MAX_INPUT_TOKENS` in `share/router/router.py`). KV cache at 4-bit + turboquant takes roughly 15–20 GB of wired RAM at this setting. Drop to 60K if your machine has less than 64 GB total or you're running other heavy apps alongside.
- **One-resident-at-a-time with polite eviction.** When a request arrives for a model that isn't resident, the router waits for the *other* model's in-flight requests to drain (refcount goes to zero), then evicts it before launching the new one. Active streams are never killed mid-response. If the other backend doesn't drain within 600 s the router evicts anyway. An idle sweeper also evicts a backend after 30 minutes (`IDLE_EVICTION_SECONDS = 1800`) of no traffic. Net effect: model switches feel snappy when nobody else is using the other model, but RAM never blows past one model's footprint.
- **Why this matters on a 64 GB box.** With a 96K input cap, KV cache per model can be 15–20 GB wired. Two big models resident simultaneously would push past 64 GB and force the OS into swap (load average >5, `httpx.RemoteProtocolError: peer closed connection without sending complete message body` in router.log, streaming dies mid-response). The polite-eviction policy above is what prevents that. If you ever observe the symptoms, check `sysctl vm.swapusage` — recovery is killing both `mlx_vlm.server` PIDs and bouncing the router (launchd respawns it).
